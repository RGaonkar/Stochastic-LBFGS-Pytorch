...
class TestOptim(TestCase):
    ...
    def _test_rosenbrock(self, constructor, old_fn):
        ...
        params_t = torch.Tensor([20 , 20])
        state = {}
        # print("size of tensor", params_t.size()) 
        params = Variable(torch.Tensor([-10, -10]),requires_grad=True)
        optimizer = constructor([params])


        solution = torch.Tensor([0 , 0])
        ...
        def eval():
            ...
            # print("rosenbrock value", loss)
            # print("Value returned by rosenbrock")
            # print(loss)
            # print("----------------------------------------------------------------------------")
            # print()
        ...
        for i in range(50):
            optimizer.step(eval)
            # print("parameters outside lbfgs", params)
            old_fn(lambda _: (rosenbrock(params_t), drosenbrock(params_t)),
                   params_t, state)
        #     self.assertEqual(params.data, params_t)
        # print("Final Params", params)
        all_losses = optimizer.all_losses
        # print("loss:")
        # print(all_losses)
        print ("gradient norm square:")
        print(optimizer.all_grad_square)
        #plot the loss
        # plt.figure()
        # plt.plot(all_losses)
        ...
        # pickle.dump(optimizer.all_grad_square, open("loss_ADAM_grad_sq.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_SdLBFGS_p_200.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_ADAM_simple_rosenbrock.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_SGD_simple_rosenbrock.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_SdLBFGS_rosenbrock.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_SdLBFGS_simple_cubic.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_SGD_simple_quad.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_SGD_simple_cubic.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_ADAM_simple_quad.pkl", "wb"))
        # pickle.dump(all_losses, open("loss_ADAM_simple_cubic.pkl", "wb"))
    ...
    # def test_adam(self):
    #     self._test_rosenbrock(
    #         lambda params: optim.Adam(params, lr=1e-3),
