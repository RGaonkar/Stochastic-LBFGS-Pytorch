    ...
    def step(self, closure):
        ...
        while n_iter < max_iter:
            ...
            if state['n_iter'] == 1:
                ...
                loss_previous = closure()
                flat_grad_prev = self._gather_flat_grad()
                ...
                loss = float(closure())  #----------------------update ---------------?
                # print("Loss is:")
                # print(loss)
                # self.all_losses.append(loss)
                # loss = float(closure)
                flat_grad = self._gather_flat_grad()
                # print("loss inside lbfgs from closure and flat_grad",loss,flat_grad)
                abs_grad_sum = flat_grad.abs().sum()
                ...
                ys = y.dot(s)  # y*s
                constant_del = 0.01
                gamma = max([y.dot(y)/s.dot(y), constant_del])
                ...
                # if True:
                    ...
                    s_reshape = torch.reshape(s, (s.size()[0], 1))

                    term2_part1 = torch.mul(s, gamma)
                    

                    term2 = 0.25 * term2_part1.dot(s)
                    
                    if (term1 < term2):
                        print ("entered")
                        numerator = 0.75 * term2 
                        denominator = term2 - term1
                        theta = numerator / denominator

                    else:
                        theta = 1

                    y_part2 = (1-theta) * torch.mul(s,gamma)
                    y_part1 = (theta * y)
        ...
        # print (type(torch.norm(flat_grad)))
        # print (type(torch.norm(flat_grad).item()))

        self.all_grad_square.append((torch.norm(flat_grad)**2).item())
        return orig_loss